...
'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
        ...
        'Referer' : 'http://www.dianping.com/shop/32501719'
}

cookie = {'_lxsdk_cuid': '16613af30c6c8-0bbab5b3309444-34677908-13c680-16613af30c7c8', 'ctu': 'a373e4ecd16dc1ef6e21629f8401f0e8e2f087f373fedff852314a6dad9daeec', '_lxsdk_s': '16840e6cfcd-932-231-893%7C%7C429', 'dper': '89d682e9c4a482b0ba876ecec8dea96afcd8d057de4869472c4c3d706a631acd003ad1ccca8a029bbf890b60648fe9a6e070a6f4508723203ab575ec5b534a5a99132abec73603f248576542d79bf98f722e298c0c2f3246be451fcf6b67a39d', '_lx_utm': 'utm_source%3DBaidu%26utm_medium%3Dorganic', 'll': '7fd06e815b796be3df069dec7836c3df', 's_ViewType': '10', 'cy': '4', 'cye': 'guangzhou', '_lxsdk': '16613af30c6c8-0bbab5b3309444-34677908-13c680-16613af30c7c8', 'ua': 'KANTEHA', '_hc.v': '""2985187f-852b-4e87-875a-f134f47ae306.1537928174""'}
...
def get_msg():
    ...
    :return:
    """
    filename = '/Users/gandehua/Desktop/dazong3.txt'   ######<==========================改这里（你的文件目录，你多建几个比如xx1..xx2然后到时复制粘贴到一个文件）

    with open(filename, 'wb') as f:
        for i in range(50,100):                ######<===============改这里（这里是页数，大众点评爬两页又ban你的，你就两页两页咁修改比如（1，3）（3，5）两页两页咁爬）

            url = 'http://www.dianping.com/shop/95240689/review_all/p'+str(i)   ######<===============改这里（这里的网址对应你的饭店网址）
            # url = "https://www.dianping.com/shop/77307732/review_all"
            html = requests.get(url, headers=header_pinlun, cookies=cookie)
            print(str(i)+" ===> STATUS", html.status_code)
            doc = pq(html.text)
            # 解析每条评论
            ...
            for data in pinglunLi:
                ...
                # print("pinglun:")
                f.write('=======================================================================\n'+css_decode(dict_css_x_y, dict_svg_text, list_svg_y, pinglun)+'\n')
...
def svg_dict(csv_html):
    ...
    for data in svg_text_re:
        ...
        # print data[2].encode('utf-8')
        dict_avg[data[0].decode('unicode_escape').replace("#", "")] = list(data[2])
    # print dict_avg
    """
...
def css_decode(css_html, svg_dict, svg_list, pinglun_html):
    ...
    newtxt = pinglun_html.replace('<span class="', ',').replace('"/>', ",").replace('">', ",")
    newtxt = remove_emoji(newtxt)
    pinglun_text = pq(newtxt).text()
    # print pinglun_text
    ...
    for x in pinglun_str:
        str_pinglun += x
    # print str_pinglun.encode('utf-8')
    return str_pinglun.encode('utf-8')
